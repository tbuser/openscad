...
if [ `uname | grep OpenBSD` ];
	...
	sudo pkg_add -v qt4-corelib qt4-gui qt4-moc qt4-opengl qt4-qmake qt4-ui
	sudo pkg_add -v	qt4-rcc xorg libGLU libXmu libXi xorg-vfbserver gmake
	sudo pkg_add -v qt4 
	sudo pkg_add -v bison flex cmake git bash
	...
	echo BASEDIR=/usr/local ./scripts/uni-build-dependencies.sh opencsg
fi

if [ `uname | grep FreeBSD` ];
